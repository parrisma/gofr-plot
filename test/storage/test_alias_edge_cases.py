...
class TestAliasGuidInteraction:
    ...
    def test_register_alias_for_nonexistent_guid(self, storage):
        """Registering alias for non-existent GUID should fail"""
        fake_guid = "00000000-0000-0000-0000-000000000000"
        
        with pytest.raises(ValueError, match="GUID .* not found|does not exist"):
    ...
    def test_delete_image_removes_alias(self, storage, sample_image_data):
        """Deleting an image should also remove its alias"""
        guid = storage.save_image(sample_image_data, "png", "testgroup")
...
class TestAliasPersistence:
    ...
    def test_alias_in_metadata_file(self, sample_image_data, tmp_path):
        """Alias should be stored in metadata.json"""
        storage = FileStorage(storage_dir=str(tmp_path))
        guid = storage.save_image(sample_image_data, "png", "testgroup")
        storage.register_alias("in-metadata", guid, "testgroup")
        
        # Check metadata file directly
        metadata_path = tmp_path / guid / "metadata.json"
        assert metadata_path.exists()
        
        with open(metadata_path) as f:
            metadata = json.load(f)
        
        assert metadata.get("alias") == "in-metadata"
...
class TestAliasErrorRecovery:
    ...
    def test_corrupted_metadata_alias_recovery(self, sample_image_data, tmp_path):
        ...
        with open(metadata_path, "w") as f:
            json.dump({"alias": 12345, "format": "png", "group": "testgroup"}, f)  # alias should be string
        
        # New storage should handle gracefully
        storage2 = FileStorage(storage_dir=str(tmp_path))
        
        # Should not crash, alias may be None or recovered
        alias = storage2.get_alias(guid)
        # Either None (corrupted) or somehow recovered
    ...
    def test_orphaned_alias_in_map(self, sample_image_data, tmp_path):
        ...
        # Manually delete the image directory (simulate corruption)
        import shutil
        shutil.rmtree(tmp_path / guid)
        
        # Storage should handle gracefully
        # Resolve should return None for orphaned alias
        # (behavior depends on whether storage re-validates on resolve)
        result = storage.resolve_identifier("orphan-test", "testgroup")
        # Result may be the (now invalid) GUID or None
        # Either is acceptable for corrupted state
